...
const UpdateProductForm = ({ slug }: { slug: string }) => {
  ...
  const { data: settingsData, isLoading: isSettingsDataLoading } =
    useSettings();
  const { mutateAsync: updateProductById, isPending: isUpdateProductPending } =
    useUpdateProduct();
  ...
  useEffect(() => {
    if (productData?.data) {
      ...
      form.reset({
        ...
        fabric: p.fabric || "",
        season: p.season || "",
        brand: p.brand || "Elena Fashion Store",
  ...
  const onSubmit = async (data: FormData) => {
    if (!productData?.data?._id) return;
  ...
  if (isUserDataLoading || isProductDataLoading) {
    return (
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-[400px] bg-destructive/10 rounded-lg border border-destructive/20 p-8">
    ...
    <Card className="w-full max-w-5xl mx-auto shadow-xl border-t-4 border-t-primary">
      <CardHeader className="bg-muted/30 pb-0 pt-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <CardTitle className="text-3xl font-bold">تحديث المنتج</CardTitle>
            <CardDescription className="text-lg">
              عدل تفاصيل المنتج المتنوعة بعناية
            </CardDescription>
          </div>
                    ...
                    <Field>
                      ...
                      <RadioGroup
                        ...
                        onValueChange={(val) => {
                          ...
                          if (
                            val === "3d" &&
                            form.getValues("colors").length > 1
                          ) {
                            const firstColor = form.getValues("colors")[0];
                            form.setValue("colors", [firstColor]);
                          } else if (
                            val === "3d" &&
