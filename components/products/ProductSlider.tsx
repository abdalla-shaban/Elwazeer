...
const ProductSlider = ({ title, subtitle, products }: ProductSliderProps) => {
  if (!products || products.length === 0) return null;

  return (
    <section className="relative overflow-hidden py-12 px-4 md:px-8 rounded-[4rem]">
          ...
          <CarouselContent className="-ml-4 md:-ml-8 cursor-grab active:cursor-grabbing">
            <AnimatePresence mode="popLayout">
              {products.map((product, index) => (
                <CarouselItem
                  key={product._id}
                  className="pl-4 md:pl-8 basis-[85%] sm:basis-1/2 lg:basis-1/3 xl:basis-1/4"
                >
                  <motion.div
